...
class MainActivity : CordovaActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (intent?.extras?.getBoolean("cdvStartInBackground", false) == true) {
            moveTaskToBack(true)
        }
        ...
        root.setId(rootId)
        setContentView(root)
        getPackageJsonPath()?.let { launchBlocking(it) }
    }
    ...
    private fun getPackageJsonPath() =
            getPackageJsonFromCordovaConfig() ?: getPackageJsonFromBuildConfig()

    private fun getPackageJsonFromCordovaConfig() = launchUrl?.takeIf { it.endsWith("package.json") }

    private fun getPackageJsonFromBuildConfig(): String? {
        return try {
            assets.open(BuildConfig.JS_APP).close()
            BuildConfig.JS_APP
        } catch (e: IOException) {
            null
        }
...
//    Snackbar.make(findViewById(rootId), message, Snackbar.LENGTH_LONG).show()
    ...
    override fun dispatchKeyEvent(event: KeyEvent) =
            getEvents()?.notifyDispatchKeyEvent(event)?.takeIf { it }
