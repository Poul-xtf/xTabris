// Type definitions for Tabris.js 3.6.1
...
export class AlertDialog extends Popup {
   ...
   * @constant
   */
  readonly jsxAttributes: JSXAttributes<this> & {children?: Flatten<string|TextInput>, textInputs?: never};
...
export namespace widgets {
  ...
  export class CollectionView<CellWidgetType extends Widget = Widget> extends Composite<CellWidgetType> {
     ...
     * @constant
     */
    readonly jsxAttributes: JSXAttributes<this> & Partial<Record<'firstVisibleIndex' | 'lastVisibleIndex', never>> & Partial<Pick<this, 'cellHeight' | 'cellType' | 'createCell' | 'updateCell'>>;
...
export namespace widgets {
  ...
  export class Composite<ChildType extends Widget = Widget> extends Widget {
     ...
     */
    protected _apply<Target = this>(options: {mode?: 'default'|'strict', trigger?: ListenersKeysOf<Target> | 'update' | symbol}, rules: ({[selector: string]: object})|((widget: this) => ({[selector: string]: object}))|null): this;
     ...
     * @param rules The ruleset to apply. May also be given as a callback which is passed to the widget instance and must return the actual ruleset.
     */
    protected _apply(rules: ({[selector: string]: object})|((widget: this) => ({[selector: string]: object}))): this;
     ...
     * @param rules The ruleset to apply. May also be given as a callback which is passed to the widget instance and must return the actual ruleset.
     */
    protected _apply(mode: 'default' | 'strict', rules: ({[selector: string]: object})|((widget: this) => ({[selector: string]: object}))): this;
    ...
    /**
     * Applies the given attributes to all descendants that match the associated selector(s). Each
     * attributes object may contain properties to be set and listeners to be registered. An entry will be
     * treated as a listener if it follows the naming scheme "onEventType". IMPORTANT: Listeners previously
     * registered (for the same type) via the `apply` method, a [JSX](../declarative-ui.md) element
     * attribute or a [widget factory](./utils.md#asfactoryconstructor) call will be de-registered. This
     * means you can call apply repeatedly and have a deterministic outcome. Listeners registered
     * programmatically (e.g. `widget.onTap(...)` are not affected by this.) <br/><br/>For better type
     * safety enable [strict mode](#applymode-rules) and use the [`Set`](./utils.md#settarget-attributes) to
     ...
     * on their parent. See "[Encapsulation](../selector.md#encapsulation)".
     * @param options If mode is set to `'strict'` the function checks that all selector match at least one widget, and that id selector match exactly one widget. <br/>A `trigger` is string to be associated with the given rulset. If set to `'update'`, the ruleset will be applied once immediately and then again every time `apply('update')` is called. If set to any event-attribute name, such as `'onTap'`, it will automatically re-apply the ruleset when this event is triggered.
     ...
     */
    apply<Target = this>(options: {mode?: 'default'|'strict', trigger?: ListenersKeysOf<Target> | 'update' | symbol}, rules: ({[selector: string]: object})|((widget: this) => ({[selector: string]: object}))|null): this;

    /**
     * Shorthand for `apply({mode: 'default'}, rules)`
     * @param rules The ruleset to apply. May also be given as a callback which is passed to the widget instance and must return the actual ruleset.
     */
    apply(rules: ({[selector: string]: object})|((widget: this) => ({[selector: string]: object}))): this;
     ...
     * @param rules The ruleset to apply. May also be given as a callback which is passed to the widget instance and must return the actual ruleset.
     */
    apply(mode: 'default' | 'strict', rules: ({[selector: string]: object})|((widget: this) => ({[selector: string]: object}))): this;
...
export namespace widgets {
  ...
  export class Picker extends Widget {
     ...
     * @constant
     */
    readonly jsxAttributes: JSXAttributes<this> & Partial<Pick<Picker, 'itemText'>>;
...
export namespace widgets {
  ...
  export class Video extends Widget {
     ...
     * @constant
     */
    readonly jsxAttributes: JSXAttributes<this> & Partial<Record<'speed' | 'position' | 'duration' | 'state' | 'children', never>>;
 ...
 * for the [`apply`](./Composite##applyproperties) method. The benefit of using this method as opposed
 * to passing the object directly is twofold: <br/><br/>1 - The IDE and/or TypeScript compiler can check
 * that the properties are matching the given widget type. <br/>2 - The `apply` method will only set the
 * properties if the selected widget matches the type given here. If there is a mismatch an error will
 * be thrown.
 * @param target The type of the target widget.
 * @param attributes A set of properties for the given widget type.
 */
export function Set<WidgetConstructor extends BaseConstructor<Widget>>(target: WidgetConstructor, attributes: Attributes<WidgetConstructor['prototype']>): Attributes<WidgetConstructor['prototype']>;
