...
function updateProjectAccordingTo (platformConfig, locations) {
    ...
    var preferencesSplashTheme = platformConfig.getPreference('ThemeSplash', 'android');
    var splashTheme = preferencesSplashTheme ? preferencesSplashTheme : '@style/Theme.Tabris.Light.SplashScreen';

    manifest.getActivity()
        .setOrientation(platformConfig.getPreference('orientation'))
        .setSplashTheme(splashTheme)
        .setLaunchMode(findAndroidLaunchModePreference(platformConfig));

    var preferencesTheme = platformConfig.getPreference('Theme', 'android');
    var theme = preferencesTheme ? preferencesTheme : '@style/Theme.Tabris.Light.DarkAppBar';
    events.emit('verbose', 'Setting application theme: ' + theme);
    manifest.setApplicationTheme(theme);
    ...
    var kotlinPattern = path.join(locations.javaSrc, manifestId.replace(/\./g, '/'), '*.kt');
    var kotlin_files = shell.ls(kotlinPattern).filter(function (f) {
        return shell.grep(/:\s+CordovaActivity/g, f);
    });

    if (kotlin_files.length === 0) {
        throw new CordovaError('No Java files found that extend CordovaActivity.');
    } else if (kotlin_files.length > 1) {
        events.emit('log', 'Multiple candidate Java files that extend CordovaActivity found. Guessing at the first one, ' + kotlin_files[0]);
    }

    var destFile = path.join(locations.root, 'app', 'src', 'main', 'java', androidPkgName.replace(/\./g, '/'), path.basename(kotlin_files[0]));
    shell.mkdir('-p', path.dirname(destFile));
    shell.sed(/package [\w\.]*;/, 'package ' + androidPkgName + ';', kotlin_files[0]).to(destFile);
    ...
    if (removeOrigPkg) {
        ...
        shell.rm('-Rf', kotlin_files[0]);
        // remove any empty directories
        var currentDir = path.dirname(kotlin_files[0]);
