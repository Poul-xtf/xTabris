...
var androidXMappings = require("./androidx-mapping.js").mapping;
...
var handlers = {
    'source-file': {
        install: function (obj, plugin, project, options) {
            ...
            migrateSupportLibraryClassesToAndroidX(project, dest);
        },
        uninstall: function (obj, plugin, project, options) {
            var dest = getInstallDestination(obj);

            // TODO: Add Koltin extension to uninstall, since they are handled like Java files
            if (obj.src.endsWith('java') || obj.src.endsWith('.kt')) {
...
function migrateSupportLibraryClassesToAndroidX(project, dest) {
  var absoluteDest = path.resolve(project.projectDir, dest);
  var sourceFile = fs.readFileSync(absoluteDest, 'utf-8');
  for (var supportLibraryClass in androidXMappings) {
    if (androidXMappings.hasOwnProperty(supportLibraryClass)) {
      if (sourceFile.indexOf(supportLibraryClass) !== -1) {
        var androidXClass = androidXMappings[supportLibraryClass];
        events.emit('log', 'Migrating plugin source file ' + absoluteDest + ' support library class "' +
          supportLibraryClass + '" to androidx class "' + androidXClass + '"');
        sourceFile = sourceFile.replace(new RegExp(supportLibraryClass, 'g'), androidXClass)
      }
    }
  }
  fs.writeFileSync(absoluteDest, sourceFile);
}
